...
- **point_cloud_compression_topic_denylist**: List of regular expressions ([ECMAScript grammar](https://en.cppreference.com/w/cpp/regex/ecmascript)) of topic names delivered with their original schema over remote access instead of being transparently Draco-compressed. Use this for point cloud topics whose exact values or point order must be preserved, since the default compression is lossy (kd-tree encoding with positions quantized to 12 bits, which also reorders points). Defaults to `[]` (all compressible point cloud topics are compressed); the launch file defaults this argument to `['(?!)']`, a never-matching pattern, because an empty array cannot be passed as a string array through a launch file. Compression applies to `sensor_msgs/msg/PointCloud2` topics and to channels carrying protobuf-encoded `foxglove.PointCloud` messages. Channels classified as Reliable skip compression automatically and deliver the raw point cloud on the control bytestream.
- **point_cloud_compression_quantization_bits**: Quantization bits for point cloud positions when they are transparently Draco-compressed over remote access, between `1` and `30`. Fewer bits compress smaller but quantize coarser: the quantization step is the cloud's position range divided by 2^bits, so e.g. a cloud spanning 100 m at the default of `12` bits resolves to about 2.4 cm. Defaults to `12`, matching the SDK default. Applies to every compressed point cloud topic; use `point_cloud_compression_topic_denylist` to opt topics out of compression entirely.
