 ...
 */
export function generatePySchemaStub(
  schemas: FoxgloveSchema[],
  { unionName = "FoxgloveSchema" }: { unionName?: string } = {},
): string {
  const header =
    [
      "# Generated by https://github.com/foxglove/foxglove-sdk",
  ...
  const typeUnion = `\n${unionName} = Union[${allSchemas.join(", ")}]\n`;

  return [header, enumMarker, ...enumSources, classMarker, ...classSources, typeUnion].join("\n");
}

/**
 * Generate the canonical messages module __init__ file.
 */
export function generatePyMessageModule(schemas: FoxgloveSchema[]): string {
...
/**
 * Generate the deprecated schemas module __init__ file that re-exports from messages.
 */
export function generatePySchemaModule(schemas: FoxgloveSchema[]): string {
  ...
  const headers = [
    `"""`,
    `Deprecated: Use :py:mod:\`foxglove.messages\` instead.`,
    "",
    `This module contains the definitions of the well-known Foxglove message types for logging.`,
    "",
    "Log messages to a corresponding channel type from :py:mod:`foxglove.channels`.",
