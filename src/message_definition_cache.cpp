...
std::string remove_action_subtype(const std::string action_type) {
  const auto action_subtype_suffixes = {
    ...
    std::string(ACTION_RESULT_SERVICE_SUFFIX) + SERVICE_EVENT_MESSAGE_SUFFIX,
    ...
    std::string(ACTION_GOAL_SERVICE_SUFFIX) + SERVICE_EVENT_MESSAGE_SUFFIX,
  };
...
const MessageSpec& MessageDefinitionCache::load_message_spec(
  ...
  if (subfolder == "action") {
    ...
    const std::string action_type_prefix = package + "/action/" + action_name;

    // The <Action>_SendGoal_Event and <Action>_GetResult_Event messages are generated by rosidl
    // for service introspection of the action's implicit services and have no definition files of
    // their own. Synthesize them from the corresponding request and response definitions,
    // mirroring rosidl's generated fields.
    const auto make_event_definition = [&action_type_prefix](const std::string& service_suffix) {
      const std::string service_type = action_type_prefix + service_suffix;
      return "service_msgs/msg/ServiceEventInfo info\n" + service_type +
             SERVICE_REQUEST_MESSAGE_SUFFIX + "[<=1] request\n" + service_type +
             SERVICE_RESPONSE_MESSAGE_SUFFIX + "[<=1] response\n";
    };
    ...
    const std::map<std::string, std::string> action_type_definitions = {
      ...
      {std::string(ACTION_RESULT_SERVICE_SUFFIX) + SERVICE_RESPONSE_MESSAGE_SUFFIX,
       "int8 status\n" + resultDef},
      {std::string(ACTION_RESULT_SERVICE_SUFFIX) + SERVICE_EVENT_MESSAGE_SUFFIX,
       make_event_definition(ACTION_RESULT_SERVICE_SUFFIX)},
      {std::string(ACTION_GOAL_SERVICE_SUFFIX) + SERVICE_REQUEST_MESSAGE_SUFFIX,
       "unique_identifier_msgs/UUID goal_id\n" + goalDef},
      {std::string(ACTION_GOAL_SERVICE_SUFFIX) + SERVICE_RESPONSE_MESSAGE_SUFFIX,
       "bool accepted\nbuiltin_interfaces/msg/Time stamp"},
      {std::string(ACTION_GOAL_SERVICE_SUFFIX) + SERVICE_EVENT_MESSAGE_SUFFIX,
       make_event_definition(ACTION_GOAL_SERVICE_SUFFIX)}};
    ...
    // The <Srv>_Event message is generated by rosidl for service introspection and has no
    // definition file of its own. Synthesize it from the request and response definitions,
    // mirroring rosidl's generated fields.
    // See https://github.com/ros2/rosidl/blob/rolling/rosidl_adapter/rosidl_adapter/parser.py
    const std::string request_type_name =
      package + "/srv/" + service_name + SERVICE_REQUEST_MESSAGE_SUFFIX;
    const std::string response_type_name =
      package + "/srv/" + service_name + SERVICE_RESPONSE_MESSAGE_SUFFIX;
    const std::string eventDef = "service_msgs/msg/ServiceEventInfo info\n" + request_type_name +
                                 "[<=1] request\n" + response_type_name + "[<=1] response\n";

    const std::map<std::string, std::string> service_type_definitions = {
      {SERVICE_REQUEST_MESSAGE_SUFFIX, requestDef},
      {SERVICE_RESPONSE_MESSAGE_SUFFIX, responseDef},
      {SERVICE_EVENT_MESSAGE_SUFFIX, eventDef}};

    // Create a MessageSpec instance for the request, response and event subtypes and add them to
    // the cache.
